...
function calculateBalanceForPaymentMethod() {
    ...
    // $productNoInput.on('keydown', function(event) {
    //     // Block all keyboard input except essential keys
    //     const allowedKeys = [
    //         8,   // Backspace
    //         9,   // Tab
    //         27,  // Escape
    ...
    //     // Allow Ctrl+A, Ctrl+C, Ctrl+V, Ctrl+X for basic operations
    //     if (event.ctrlKey && [65, 67, 86, 88].includes(event.keyCode)) {
    //         return true;
    //     }
        
    //     // Block all other keys (including numbers, letters, Enter)
    ...
    // // Handle barcode scanner input (usually comes as a rapid sequence ending with Enter)
    // let barcodeBuffer = '';
    // let barcodeTimeout = null;

    // $productNoInput.on('input', function(event) {
    ...
    //     // Only process if input looks like it came from a scanner (rapid input)
    //     barcodeTimeout = setTimeout(() => {
    //         if (currentValue.length >= 3) { // Minimum barcode length
    //             fetchProductDetails(currentValue);
    //             $(this).val(''); // Clear after processing
    //         }
    //     }, 100); // 100ms delay to capture complete barcode
    // });

    // // Alternative: Listen for paste events (some scanners simulate paste)
    // $productNoInput.on('paste', function(event) {
    //     event.preventDefault();
    //     const pastedData = (event.originalEvent.clipboardData || window.clipboardData).getData('text');
        
    //     if (pastedData && pastedData.length >= 3) {
    //         fetchProductDetails(pastedData);
    //         $(this).val('');
    //     }
    // });

    // // Add visual indicator that field is barcode-only
    // $productNoInput.attr('placeholder', 'Scan Product Barcode');

    // // Optional: Add a visual scanning effect
    // $productNoInput.on('focus', function() {
    //     $(this).addClass('barcode-scanning');
    // });

    // $productNoInput.on('blur', function() {
    //     $(this).removeClass('barcode-scanning');
    // });
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // Enhanced barcode functionality - add this to your existing script

// Override the print button click event
$('#printButton').off('click').on('click', function(e) {
    ...
    @else
    // For non-staff users (superadmin), submit the form directly
...
function calculateSmartGoldAdvanceAmount() {
    ...
    if (adGoldGrams > totalProductGrams) {
        ...
        showGoldCalculationInfo(`
            <div class="card border-info shadow-sm">
                <div class="card-body p-3 text-dark">
                    ${userEnteredRate > 0 ? `<div class="alert alert-info p-2 mb-2"><small><strong>Using custom rate:</strong> Rs ${userEnteredRate.toFixed(2)}/g</small></div>` : ''}
                    <div class="d-flex justify-content-between mb-2">
                        <span>Total AD Gold</span>
        ...
        if (userEnteredRate > 0) {
            showGoldCalculationInfo(`
                <div class="alert alert-info p-2 mb-2">
                    <small><strong>Using custom rate:</strong> Rs ${userEnteredRate.toFixed(2)}/g</small>
                    <br>
                    <small>${adGoldGrams.toFixed(3)} g × Rs ${effectiveRate.toFixed(2)} = Rs ${calculatedAmount.toFixed(2)}</small>
                </div>
            `);
        } else {
...
$(document).on('input change', '#GoldRatePerGram', function() {
    // Recalculate gold advance if it's being used
    if (currentAdvanceUsage.gold.used) {
        const newGoldAmount = calculateSmartGoldAdvanceAmount();
        currentAdvanceUsage.gold.amount = newGoldAmount;
        
        // Update the user-entered rate in storage
        ...
        // Update the gold advance display
    ...
    $('#orderCashContent').html(`
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="useOrderCashAmount"
                  max="${maxUsableCash}" min="0" step="0.01" value="${Math.max(0, maxUsableCash).toFixed(2)}">
        </div>
    `);
    ...
    $('#orderGoldContent').html(`
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="useOrderGoldAmount"
                  max="${goldAdvance}" min="0" step="0.001" value="${goldAdvance.toFixed(4)}">
        </div>
    `);
}
