...
//------------------------------------------------------------
    ...
    // ============== Staff Barcode input & walk in customer credit invoice validation ==============

    $('#printButton').off('click').on('click', function(e) {
        e.preventDefault();
        
        // Validate walk-in customer credit invoice
        const customerId = $customerSelect.val();
        const balanceAmount = parseFloat($('#balance').val()) || 0;
        
        // Check if it's walk-in customer (ID = 1) with positive balance (credit)
        if (customerId == '1' && balanceAmount > 0) {
            Swal.fire({
                title: 'Invalid Transaction',
                text: 'Cannot process credit invoice for walk-in customer. In order to process select a registered customer.',
                icon: 'error',
